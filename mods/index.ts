...
function resolveMemoryDir(ctx, home) {
  ...
  if (agentId) {
    const localCandidate = path.join(home, ".letta", "lc-local-backend", "memfs", String(agentId), "memory");
    if (fs.existsSync(localCandidate)) return { path: localCandidate, source: "ctx.agent.id (local backend)" };

    const cloudCandidate = path.join(home, ".letta", "agents", String(agentId), "memory");
    if (fs.existsSync(cloudCandidate)) return { path: cloudCandidate, source: "ctx.agent.id (cloud agent)" };
  }
...
function detectEnvironment(ctx) {
  ...
  if (IS_WIN) {
    label = "Desktop / local Windows";
  } else if (markers.some((m) => m.includes("Railway")) && !markers.some((m) => m.includes("Desktop"))) {
...
/**
 * On Windows, check if a file exists and is executable.
 * `fs.constants.X_OK` is unreliable on Windows (returns true for everything),
 * so we check by extension (PATHEXT) and then verify access.
 */
function isExecutable(candidate) {
  // On Windows, any file with a PATHEXT extension is executable.
  // For files without extension (e.g. `letta` with no .cmd), fall back to access check.
  if (IS_WIN) {
    ...
    // a symlink or shim without extension — ask access
    try {
...
/**
 * Enumerate the given executable name across every PATHEXT suffix.
 * On non-Windows, behaves exactly as before (exact name match).
 */
function collectExecutableCandidates(name) {
...
function findPathExecutables(name) {
  const seen = new Set();
  const results = [];
  const executables = collectExecutableCandidates(name);
  for (const part of String(process.env.PATH || "").split(path.delimiter).filter(Boolean)) {
    for (const candidateName of executables) {
...
function findWindowsDesktopPackage() {
  // Typical paths for Letta Desktop on Windows
  const localAppData = process.env.LOCALAPPDATA || "";
  const candidates = [
    path.join(localAppData, "Programs", "letta-code", "resources", "app.asar.unpacked", "node_modules", "@letta-ai", "letta-code", "package.json"),
    path.join(localAppData, "Programs", "letta-code", "resources", "app.asar", "node_modules", "@letta-ai", "letta-code", "package.json"),
  ];
...
async function lettaRuntimeSummary() {
  ...
  const versions = await Promise.all(paths.map(async (candidate) => {
    ...
    const isMacApp = candidate.includes("/Applications/Letta.app/Contents/MacOS/letta");
    const isWindowsApp = candidate.toLowerCase().includes("letta-code") && candidate.endsWith(".exe");
    let note = "";
    if (isMacApp) note = " (Desktop launcher; --version may report Electron/Node)";
    if (isWindowsApp) note = " (Desktop launcher)";
    return {
      path: candidate,
  ...
  // Find bundled desktop package — try Windows first, then macOS
  let desktopPackagePath = findWindowsDesktopPackage();
  if (!desktopPackagePath) {
    desktopPackagePath = "/Applications/Letta.app/Contents/Resources/app.asar.unpacked/node_modules/@letta-ai/letta-code/package.json";
  }
